...
def add_edge_brightening(img: np.ndarray, strength: float) -> np.ndarray:
    """Edge brightening / geometric secondary-electron (SE) yield enhancement.

    Physical justification: when the primary beam scans near a sharp
    vertical sidewall, a larger fraction of the SE excitation volume lies
    within the escape depth, sharply raising the local SE yield -- SEM
    images are therefore brighter along feature edges (Seiler 1983;
    Cazaux 2012). Implemented as a multiplicative brightness boost
    proportional to the *normalized* gradient magnitude. It is applied at
    native resolution *before* any noise injection so the edge signal is
    genuine image content; the 10x downsample of the search image then
    averages it exactly as real low-magnification imaging would.
    """
    if strength <= 0:
        return img
    img_f = img.astype(np.float64)
    ...
    out = img_f * (1.0 + strength * mag_norm)
    return np.clip(out, 0, 255).astype(np.uint8)


def apply_rotation_scale(img: np.ndarray, rotation_deg: float, scale: float) -> np.ndarray:
    """Stage-rotation + magnification-calibration errors, applied about the
    image centre so the final frame stays exactly 1000x1000 (a real tool
    re-centres its field of view). `rotation_deg` models small scan-field
    rotation (stage/magnetic alignment drift); `scale` models magnification
    calibration drift (typically within ~2%). Because this warp moves the
    reference content, the ground-truth location must be transformed by the
    identical affine matrix (see `transform_point` / pipeline) so the
    recorded GT remains the true location in the *final* image.
    """
    if rotation_deg == 0.0 and scale == 1.0:
        return img
    h, w = img.shape[:2]
    cx, cy = (w - 1) / 2.0, (h - 1) / 2.0
    M = cv2.getRotationMatrix2D((cx, cy), float(rotation_deg), float(scale))
...
def transform_point(
    rotation_deg: float, scale: float, x: float, y: float,
    img_w: int = 1000, img_h: int = 1000,
) -> tuple:
    """Apply the same affine warp as `apply_rotation_scale` to a single
    (x, y) location. Used by the pipeline to move the ground-truth centre
    and box into the final (rotated/zoomed) search-image coordinate frame.
    """
...
def image_reference(
    ...
    img = add_edge_brightening(img, edge_brightening_strength)
...
def image_search(
    ...
    brightened = add_edge_brightening(blurred, edge_brightening_strength)
    downsampled = downsample_area_average(brightened, factor)
    geometric = apply_rotation_scale(downsampled, rotation_deg, scale_search)
    drifted = apply_raster_drift(geometric, shear_amplitude_px, drift_jitter_px, rng)
