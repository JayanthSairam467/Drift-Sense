...
class GenerationParams:
    ...
    # Edge brightening / geometric secondary-electron yield enhancement
    # (mandatory SEM behaviour), applied to BOTH images at native resolution
    # before noise injection (0 = none).
    edge_brightening_strength: float = 0.0
    # Max scan-field rotation (degrees) applied to the search image about its
    # centre (stage/magnetic alignment drift). Generators randomize within
    # +/-rotation_deg; 0 = none.
    rotation_deg: float = 0.0
    # +/- magnification calibration drift (fraction), e.g. 0.02 lets a
    # generator pick scale_search in [0.98, 1.02]; 0 = none.
    scale_search_range: float = 0.0
    # Actual per-sample zoom applied to the search image (set by the
    # generator from scale_search_range; default 1.0 = none).
...
def _pick_crop_origin(zone_result: dict, params: GenerationParams, rng: np.random.Generator) -> tuple:
    # When the search image is rotated/zoomed about its centre, the reference
    # box rotates/zooms too; the crop origin must be pulled inward so the
    # whole reference box stays inside the 1000x1000 search frame (the affine
    # warp about the centre moves off-centre content outward).
    rot_margin = int(
        REFERENCE_SIZE_PX // SCALE_FACTOR * 1.0
...
def compute_gt(x0: int, y0: int, params: GenerationParams) -> tuple:
    """Ground-truth centre + box in search-image coordinates.

    By default (no rotation/zoom) this is the simple 10x mapping of the
    crop origin plus half the 100px box. When the search image is rotated
    or zoomed about its centre, the same affine transform is applied to the
    GT so the recorded coordinates remain the TRUE location in the final
    image (the challenge requires GT be exactly where the reference appears).
    """
    box_w = box_h = REFERENCE_SIZE_PX // SCALE_FACTOR  # 100
    gt_cx = x0 / SCALE_FACTOR + box_w / 2.0
    gt_cy = y0 / SCALE_FACTOR + box_h / 2.0
    if params.rotation_deg != 0.0 or params.scale_search != 1.0:
        gt_cx, gt_cy = sem_imaging.transform_point(
            params.rotation_deg, params.scale_search, gt_cx, gt_cy
        )
        box_w = box_h = (REFERENCE_SIZE_PX // SCALE_FACTOR) * params.scale_search
    gt_x0 = gt_cx - box_w / 2.0
    gt_y0 = gt_cy - box_h / 2.0
    return gt_x0, gt_y0, box_w, box_h
    ...
    *imaged* differs, which is the point: the same Reference should still be
    ...
    for i, overrides in enumerate(variants):
        ...
        search_img = sem_imaging.image_search(
            ...
            rotation_deg=variant_params.rotation_deg,
            scale_search=variant_params.scale_search,
            edge_brightening_strength=variant_params.edge_brightening_strength,
        )
